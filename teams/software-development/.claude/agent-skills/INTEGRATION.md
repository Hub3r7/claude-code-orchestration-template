...
|-------|-------------------|
| `doubt-driven-development` | **Orchestrator-level, not an agent's skill.** It needs genuine fresh-context review, which only the main session can spawn — an agent cannot nest one. Our chain already *is* doubt-driven: `quality-gate`, `hunter`, and `defender` are the fresh-context reviewers, and PASS/FAIL is the adversarial check. The orchestrator applies this skill by upgrading the tier, adding a review agent, or consulting `critic` — the on-call consultant built for exactly this fresh-context challenge, which reads this skill as the doctrine behind its role. Never by an agent self-questioning in place. (For this reason it is not in any *chain* agent's mapped set.) |
